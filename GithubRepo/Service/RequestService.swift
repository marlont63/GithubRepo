...
extension RequestService {
    public func send<Request: GithubApiRequest>(_ request: Request, success: @escaping (_ response: [GetRoutersResponse]) -> Void, failure: @escaping (_ error: GithubError) -> Void) {
        ...
        let task: URLSessionDataTask = session.dataTask(with: serverRequest) { data, urlResponse, error in
            ...
            case (let data?, let urlResponse as HTTPURLResponse, _):
                ...
                do {
                    let response = try decode.decode([GetRoutersResponse].self, from: data)
                    DispatchQueue.main.async {
                        success(response)
                    }
                } catch {
    ...
    func downloadImage(from url: URL ,success: @escaping (_ image: UIImage) -> Void, failure: @escaping (_ error: GithubError) -> Void){
        ...
        let downloadPicTask: URLSessionDataTask = session.dataTask(with: url) { (data, urlResponse, error) in
            switch (data, urlResponse, error) {
             case (let data, let urlResponse as HTTPURLResponse, let error?):
                 urlResponse.showLog(from: data)
                 DispatchQueue.main.async {
